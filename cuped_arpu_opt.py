"""CUPED sensitivity optimization for ARPU and AVG_DOWN in A/B tests.
...
Usage (ARPU + AVG_DOWN):
  python cuped_arpu_opt.py --input data.csv --user-col user_id --group-col group \
    --outcome-col arpu --pre-cols arpu_pre_7d arpu_pre_14d \
    --avg-down-col avg_down --avg-down-pre-cols avg_down_pre_7d avg_down_pre_14d
"""
...
def fit_cuped(
    ...
    if np.isnan(y).any():
        raise ValueError(f"{metric_name} outcome column contains non-numeric values")

    x_df = df[pre_cols].apply(pd.to_numeric, errors="coerce")
    if x_df.isna().any().any():
        raise ValueError(f"{metric_name} pre-period covariates contain non-numeric values")
...
def format_report(result: CupedResult) -> str:
    ...
    lines = [
        f"=== A/B {result.metric_name} CUPED 敏感度优化报告 ===",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Optimize A/B ARPU/AVG_DOWN sensitivity with CUPED")
    ...
    parser.add_argument("--outcome-col", required=True, help="Primary outcome column, e.g. arpu")
    parser.add_argument(
        "--pre-cols",
        nargs="+",
        required=True,
        help="Primary metric pre-experiment covariates",
    )

    parser.add_argument(
        "--avg-down-col",
        default=None,
        help="Optional avg_down outcome column to run a second CUPED sensitivity test",
    )
    parser.add_argument(
        "--avg-down-pre-cols",
        nargs="+",
        default=None,
        help="Optional pre-experiment covariates for avg_down metric",
    )
    ...
    primary_result = fit_cuped(
        data=data,
        group_col=args.group_col,
        outcome_col=args.outcome_col,
        pre_cols=args.pre_cols,
        metric_name=args.outcome_col.upper(),
    )
    print(format_report(primary_result))

    if args.avg_down_col is not None:
        if not args.avg_down_pre_cols:
            raise ValueError("--avg-down-pre-cols is required when --avg-down-col is provided")
        avg_down_result = fit_cuped(
            data=data,
            group_col=args.group_col,
            outcome_col=args.avg_down_col,
            pre_cols=args.avg_down_pre_cols,
            metric_name=args.avg_down_col.upper(),
        )
        print()
        print(format_report(avg_down_result))
